...
public protocol Pattern {
	...
	associatedtype Subject : BidirectionalCollection /* where Subject.Element : Equatable */										// TODO: Add constraint in Swift 4
	
	/// The type for collections of matches generated by patterns during forward matching.
	associatedtype ForwardMatchCollection : BidirectionalCollection /* where ForwardMatchCollection.Element == Match<Subject> */	// TODO: Add constraint in Swift 4
	
	/// The type for collections of matches generated by patterns during backward matching.
	associatedtype BackwardMatchCollection : BidirectionalCollection /* where BackwardMatchCollection.Element == Match<Subject> */	// TODO: Add constraint in Swift 4
...
extension Pattern where ForwardMatchCollection.Iterator.Element == Match<Subject>, BackwardMatchCollection.Iterator.Element == Match<Subject> {		// TODO: Remove redundant constraint in Swift 4
...
extension Pattern where ForwardMatchCollection.Iterator.Element == Match<String.CharacterView>, BackwardMatchCollection.Iterator.Element == Match<String.CharacterView>, Subject == String.CharacterView {
	
	// TODO: Remove extension when String conforms to BidirectionalCollection, in Swift 4
	
	/// Returns a lazily computed collection of forward matches over some subject that match the subject wholly.
	///
	/// - Complexity: O(1) to create the collection; O(b^d) to traverse it completely, where *b* is the length of the largest sequential subpattern and *d* is the largest depth of recursion.
	///
	/// - Parameter subject: The string over which to match the pattern.
	///
	/// - Returns: A lazily computed collection of forward matches of `self` over `subject`.
	public func matches(over subject: String) -> LazyFilterBidirectionalCollection<ForwardMatchCollection> {
		return forwardMatches(enteringFrom: Match(over: subject.characters, direction: .forward))
			.lazy
			.filter { candidateMatch in candidateMatch.remainingElements(direction: .forward).isEmpty }
	}
	...
	///
	/// - Returns: A lazily computed collection of backward matches of `self` over `subject`.
	public func backwardMatches(over subject: String) -> LazyFilterBidirectionalCollection<BackwardMatchCollection> {
		return backwardMatches(recedingFrom: Match(over: subject.characters, direction: .backward))
			.lazy
			.filter { candidateMatch in candidateMatch.remainingElements(direction: .backward).isEmpty }
	}
