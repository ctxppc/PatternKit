...
public protocol Pattern {
	...
	/// The type for collections of matches generated by patterns.
	...
	/// Returns a (typically lazily computed) collection of matches over a subject, given a base match and a direction.
	///
	/// - Parameter base: The match on which to base successor matches.
	/// - Parameter direction: The direction of matching.
	///
	/// - Returns: A (typically lazily computed) collection of matches over the subject in `base`, starting from the input position of `base`, and matching in the direction given by `direction`.
...
extension Pattern where MatchCollection.Iterator.Element == Match<Subject> {
	
	/// Returns a lazily computed collection of matches over some subject that match the subject wholly.
	///
	...
	///
	/// - Returns: A lazily computed collection of matches of `self` over `subject` in the direction given by `direction`.
	public func matches(over subject: Subject, direction: MatchingDirection = .forward) -> LazyFilterBidirectionalCollection<MatchCollection> {
		return matches(base: Match(over: subject, direction: direction), direction: direction).lazy.filter { candidateMatch in candidateMatch.remainingElements(direction: direction).isEmpty }
	}
	...
	public func hasMatches(over subject: Subject, direction: MatchingDirection = .forward) -> Bool {
		return !matches(over: subject, direction: direction).isEmpty
	}
	
}

extension Pattern where MatchCollection.Iterator.Element == Match<String.CharacterView>, Subject == String.CharacterView {
	...
	///
	/// - Returns: A lazily computed collection of matches of `self` over `subject` in the direction given by `direction`.
	public func matches(over subject: String, direction: MatchingDirection = .forward) -> LazyFilterBidirectionalCollection<MatchCollection> {
		return matches(base: Match(over: subject.characters, direction: direction), direction: direction).lazy.filter { candidateMatch in candidateMatch.remainingElements(direction: direction).isEmpty }
	}
	...
	///
	/// - Returns: `true` if there is at least one match of `self` over `subject`; `false` otherwise.
	public func hasMatches(over subject: String, direction: MatchingDirection = .forward) -> Bool {
		return !matches(over: subject, direction: direction).isEmpty
	}
